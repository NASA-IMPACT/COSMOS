...
class TestGetBoto3Session:
    ...
    def blank_pipeline_credentials(self):
        """Pin the pipeline credentials to their base.py defaults: a developer's
        .envs/.local may carry real SDE_AWS_* values, and these tests must not depend
        on that."""
        with override_settings(
            SDE_AWS_ACCESS_KEY_ID="", SDE_AWS_SECRET_ACCESS_KEY="", SDE_AWS_SESSION_TOKEN="", AWS_REGION="us-east-1"
        ):
            yield
    ...
    @override_settings(
        SDE_AWS_ACCESS_KEY_ID="ASIATEST", SDE_AWS_SECRET_ACCESS_KEY="secret", SDE_AWS_SESSION_TOKEN="tok"
    )
    def test_session_token_passed_for_temporary_creds(self):
        """SSO / STS credentials are a triple; the token must reach boto3 or every call is
        rejected with InvalidClientTokenId."""
        with patch("sde_collections.utils.aws.boto3.Session") as mock_session:
            get_boto3_session()
