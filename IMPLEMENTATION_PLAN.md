...
with a command override (`python3 api_scraper.py --source WEB_COSMOS --collection <config_folder> --target test|prod
...
2. **Dispatch.** Assume **`CosmosIndexingDispatchRole-{env}`** (same-account in dev — see
   ...
   with a **command override**: `python3 api_scraper.py --source WEB_COSMOS --collection {config_folder}
